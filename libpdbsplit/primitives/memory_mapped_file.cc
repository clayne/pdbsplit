...
s_memory_mapped_file_handle open_memmapped_file(const char* path)
{
	...
	{
		return s_memory_mapped_file_handle{ INVALID_HANDLE_VALUE, INVALID_HANDLE_VALUE, 0, nullptr };
	}
	...
	return s_memory_mapped_file_handle{ file, fileMapping, file_size.QuadPart, baseAddress };
#else
	struct stat fileSb;
...
s_binary_view s_memory_mapped_file::get_binary()
{
	return s_binary_view((uint8_t*)get_data(), handle.len);
}
